...
export default function DiagnosticoAprofundadoPage() {
    ...
    const processarResposta = async (respostaUsuario: string | null) => {
        ...
        try {
            ...
            if (data.status === 'finalizado') {
                setDiagnostico({ status: 'finalizado', relatorio_final: data.relatorio_final, pergunta_atual: null, resumo_etapa: null });
            } else if (data.status === 'confirmacao') {
                setDiagnostico({ status: 'confirmando', resumo_etapa: data.resumo_etapa, pergunta_atual: null, relatorio_final: null });
            } else {
                setDiagnostico({ status: 'perguntando', pergunta_atual: data.proxima_pergunta, resumo_etapa: null, relatorio_final: null });
            }
    ...
    const renderInput = (pergunta: Pergunta) => {
        switch (pergunta.tipo_resposta) {
            case 'texto':
                return <Input type="text" value={resposta} onChange={(e) => setResposta(e.target.value)} placeholder="Digite sua resposta..." className="bg-slate-700 border-slate-500 text-center text-lg" autoFocus />;
            case 'numero':
                return <Input type="number" value={resposta} onChange={(e) => setResposta(e.target.value)} placeholder="Digite um número..." className="bg-slate-700 border-slate-500 text-center text-lg" autoFocus />;
            case 'multipla_escolha':
                return (
                    <div className="flex flex-col gap-3">
                        {pergunta.opcoes?.map(opt => (
                            <Button key={opt} variant="outline" size="lg" onClick={() => processarResposta(opt)} className="justify-center h-auto py-3 whitespace-normal">
                                {opt}
                            </Button>
                        ))}
                    </div>
            ...
            case 'selecao':
                return (
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <Button type="submit" size="lg" disabled={!resposta}>Próximo</Button>
                    </div>
                );
            case 'sim_nao':
                return (
                    <div className="flex justify-center gap-4">
                        <Button onClick={() => processarResposta('Sim')} size="lg" className="bg-green-600 hover:bg-green-700">Sim</Button>
                        <Button onClick={() => processarResposta('Não')} size="lg" variant="destructive">Não</Button>
                    </div>
                );
            default:
                return <p className="text-red-400">Tipo de pergunta não suportado: {pergunta.tipo_resposta}</p>;
        }
    };
    ...
    const renderContent = () => {
        if (diagnostico.status === 'carregando') {
            return <div className="text-center text-lg">Carregando... A IA está preparando a próxima etapa.</div>;
        }
        ...
        if (diagnostico.status === 'confirmando' && diagnostico.resumo_etapa) {
            return (
                <div className="bg-slate-800 p-8 rounded-lg shadow-xl text-center space-y-6">
                    <h2 className="text-2xl font-semibold">Por favor, confirme os dados</h2>
                    ...
                    */}
                    ...
                    <div className="flex justify-center gap-4 pt-4">
                        <Button onClick={() => processarResposta('Sim')} size="lg" className="bg-green-600 hover:bg-green-700">Confirmar e Continuar</Button>
                        <Button onClick={() => processarResposta('Não')} size="lg" variant="destructive">Corrigir Informações</Button>
                    </div>
                </div>
            );
        }
        if (diagnostico.status === 'perguntando' && diagnostico.pergunta_atual) {
            ...
            return (
                <div className="bg-slate-800 p-8 rounded-lg shadow-xl text-center space-y-8">
                    <h2 className="text-2xl font-semibold leading-relaxed">{pergunta.texto}</h2>
                    <form onSubmit={(e) => { e.preventDefault(); if (resposta.trim()) processarResposta(resposta); }}>
                        <div className="space-y-4">
                            {renderInput(pergunta)}
                            {(pergunta.tipo_resposta === 'texto' || pergunta.tipo_resposta === 'numero') && (
                                <Button type="submit" size="lg" disabled={!resposta.trim()}>Próximo</Button>
                            )}
                        </div>
                    </form>
                </div>
        ...
        return <p>Ocorreu um erro ou o estado é inválido. Por favor, atualize a página.</p>;
    }
