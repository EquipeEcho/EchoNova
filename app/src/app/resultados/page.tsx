    ...
    if (data.resultados) {
      data.dimensoesSelecionadas.forEach((dimensao) => {
        ...
        if (resultado.trilhasDeMelhoria.length > 0) {
          ...
          content +=
            "  - Nenhum ponto crítico que exige ação imediata foi identificado. Ótimo trabalho!\n";
        }
        content += "\n";
      });
    } else {
...
`;
  ...
  const handleDownloadReport = () => {
    ...
    link.href = url;
    const nomeEmpresa =
      diagnosticoData.perfil.nome_empresa || diagnosticoData.perfil.empresa;
        ...
        <div className="max-w-4xl w-full bg-slate-800/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-2xl border border-white/10">
          <div className="text-center mb-8">
            ...
            <p className="text-white/80 mt-2">
              Um resumo prático sobre a maturidade de{" "}
              <span className="font-bold text-white">
                {diagnosticoData?.perfil?.empresa || "sua empresa"}
              </span>{" "}
              em áreas-chave.
            </p>
          </div>
          ...
          {diagnosticoData?.resultados ? (
            <div className="flex flex-wrap gap-6 mb-8 justify-center">
              {diagnosticoData?.dimensoesSelecionadas?.map((dimensao) => {
                ...
                return (
                  <div
                    key={dimensao}
                    className="bg-white/5 w-sm p-6 rounded-lg border border-white/10 flex flex-col"
                  >
                    <div>
                      <h3 className="text-xl font-bold text-pink-400 mb-2">{dimensao}</h3>
                      <p className="mb-4 text-white/90">
                        Sua empresa está no estágio{" "}
                        <span className="font-bold">{resultado.estagio}</span>.
                      </p>
                    </div>
                    <div className="mt-4">
                      ...
                      {resultado.trilhasDeMelhoria.length > 0 ? (
                        ...
                        <p className="text-sm text-green-400">
                          Nenhum ponto crítico identificado. Ótimo trabalho!
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-white/10 rounded-lg p-6 mb-8 text-center">
              ...
              <p className="text-white/80">
                Os resultados detalhados não puderam ser carregados. Verifique o relatório
                para ver suas respostas.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="max-w-4xl w-full bg-slate-800/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 shadow-2xl border border-white/10">
        <div className="text-center mb-8">
          ...
          <p className="text-white/80 mt-2">
            Um resumo prático sobre a maturidade de{" "}
            <span className="font-bold text-white">
              {diagnosticoData.perfil.empresa}
            </span>{" "}
            em áreas-chave.
          </p>
        </div>
        ...
        {diagnosticoData.resultados ? (
          ...
          <div className="bg-white/10 rounded-lg p-6 mb-8 text-center">
            ...
            <p className="text-white/80">
              Os resultados detalhados não puderam ser carregados. Verifique o
              relatório para ver suas respostas.
            </p>
          </div>
        )}
        ...
        <div className="space-y-4">
          <button
            ...
            className="cursor-pointer w-full px-8 py-4 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button
              ...
              className="cursor-pointer px-6 py-3 bg-gradient-to-r from-pink-500 to-pink-600 hover:from-pink-600 hover:to-pink-700 text-white font-bold rounded-lg transition-all duration-300 transform hover:scale-105"
            >
              🔄 Fazer Novo Diagnóstico
            </button>
            <button
              onClick={() => router.push("/")}
              className="cursor-pointer px-6 py-3 bg-white/20 hover:bg-white/30 text-white font-bold rounded-lg transition-all duration-300 transform hover:scale-105"
            >
