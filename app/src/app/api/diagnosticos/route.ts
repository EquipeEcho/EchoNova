...
const mapeamentoPontuacao: Record<string, number> = {
    "p1-1": 4, "p2-1": 4, "p3-1": 4, "p4-1": 4, "p5-1": 4, "p6-1": 4, "p1-2": 3, "p2-2": 3, "p3-2": 3, "p4-2": 3, "p5-2": 3, "p6-2": 3, "p1-3": 2, "p2-3": 2, "p3-3": 2, "p4-3": 2, "p5-3": 2, "p6-3": 2, "p1-4": 1, "p2-4": 1, "p3-4": 1, "p4-4": 1, "p5-4": 1, "p6-4": 1,
};
const mapeamentoMetas: Record<string, { meta: string; trilha: string }> = {
    'pergunta1': { meta: "Comunicação", trilha: "Feedback, escuta ativa" }, 'pergunta2': { meta: "Liderança", trilha: "Delegação, engajamento" }, 'pergunta3': { meta: "Criatividade", trilha: "Inovação incremental" }, 'pergunta4': { meta: "Autogestão", trilha: "Gestão de tempo, priorização" }, 'pergunta5': { meta: "Cultura & Valores", trilha: "Propósito, diversidade" }, 'pergunta6': { meta: "Transversal", trilha: "LMS, microlearning" },
};
...
function processarResultados(dimensoesSelecionadas: string[], respostasDimensoes: any) {
    ...
    for (const nomeDimensao of dimensoesSelecionadas) {
        ...
        const pontuacoesPerguntas = Object.entries(respostasDaDimensao).map(([id, valor]) => ({ id: id as string, pontuacao: mapeamentoPontuacao[valor as string] || 0, }));
        ...
        const trilhasDeMelhoria = pontuacoesPerguntas.filter(p => p.pontuacao <= 2).map(p => mapeamentoMetas[p.id]).filter(Boolean);
        const sortedScores = [...pontuacoesPerguntas].sort((a, b) => a.pontuacao - b.pontuacao);
        const fragilidade = sortedScores.length > 0 ? mapeamentoMetas[sortedScores[0].id] || null : null;
        const forca = sortedScores.length > 0 ? mapeamentoMetas[sortedScores[sortedScores.length - 1].id] || null : null;
        resultadosFinais[nomeDimensao] = { media: parseFloat(media.toFixed(2)), estagio, trilhasDeMelhoria, resumoExecutivo: { forca, fragilidade }, };
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (!dados.perfil || !dados.perfil.email || !dados.perfil.empresa) {
        return NextResponse.json({ error: "Dados de perfil incompletos. Email e nome da empresa são obrigatórios." }, { status: 400 });
    }
    ...
    const resultadosProcessados = processarResultados(dados.dimensoesSelecionadas, dados.respostasDimensoes);
    ...
    return NextResponse.json({ error: "Ocorreu uma falha interna ao processar o diagnóstico.", details: err.message }, { status: 500 });
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    const diagnosticos = await Diagnostico.find({ empresa: empresaId }).sort({ createdAt: -1 }).populate('empresa', 'nome_empresa email');
