...
export default function Diagnostico() {
  ...
  const [fase, setFase] = useState<
    "perfil" | "selecionarDimensoes" | "dimensao"
  >("perfil");
  ...
  const proximaDimensao = async (respostasAtualizadas?: DimensaoRespostas) => {
    if (indiceDimensaoAtual < dimensoesSelecionadas.length - 1) {
      ...
      console.log("Finalizado. Enviando para salvar:", {
        respostasPerfil,
        respostasDimensoes: respostasFinais,
      });
  ...
  if (fase === "dimensao") {
    const dimAtual = dimensoesSelecionadas[indiceDimensaoAtual];
    const isUltimaDimensao =
      indiceDimensaoAtual === dimensoesSelecionadas.length - 1;
