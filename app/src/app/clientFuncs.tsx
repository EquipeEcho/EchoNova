...
export function Header() {
  return (
    <div className="fixed top-4 right-4 z-50 flex gap-3">
      {/* Botão Instagram */}
      <a href="https://www.instagram.com/entre.nova/" className="social-btn" aria-label="Instagram - Entre.Nova">
          <svg
        ...
        <span className="sr-only">Instagram - Entre.Nova</span>
      </a>
      ...
      >
        ...
        <span className="sr-only">LinkedIn - Entrenova</span>
      </a>
    <DialogCloseButton />
      {/* Logo */}
...
export function DialogCloseButton() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      console.error(err);
      toast.error("Erro ao tentar fazer login. Verifique sua conexão e tente novamente.");
    }
  }
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogTrigger asChild>
        <Button className="social-btn">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="w-6 h-6 text-white" viewBox="0 0 16 16" aria-hidden="true">
      ...
      <DialogContent className="sm:max-w-md bg-gray-950 text-gray-900 border-slate-800">
        ...
        <LoginForm key={isDialogOpen ? 'open' : 'closed'} onSuccess={() => setIsDialogOpen(false)} />
